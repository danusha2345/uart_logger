...
| Пурпурный | Мигание | Тест SD-карты (запись/чтение/проверка) |
...
- При старте выполняется тест записи/чтения (512 байт) — пурпурный LED
- В режиме простоя каждые 2 сек выполняется flush для обнаружения извлечения карты
